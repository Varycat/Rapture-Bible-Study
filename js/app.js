...
function renderLibrary(){
  ...
  $('#featuredStudies').innerHTML=list.length?list.map((s,i)=>`<article class="studyCard ${i===0?'primary':''}"><div class="studyThumb${effBanners()[s.id]?' hasPhoto':''}"${effBanners()[s.id]?` style="--card-photo:url('${effBanners()[s.id]}')"`:''}><span class="thumbIcon">${iconHTML(s.id,s.icon)}</span>${s.draft?`<span class="draftBadge">${t('draftL')}</span>`:''}<button class="favBtn" data-fav="${s.id}" title="${favorites.has(s.id)?t('favDel'):t('favAdd')}">${favorites.has(s.id)?'★':'☆'}</button></div><div class="cardBody"><h3 class="thumbTitle">${s.title}</h3><div class="sub">${s.subtitle||''}</div><p>${s.description||''}</p><button data-study="${s.id}">${t('openStudyBtn')}</button></div></article>`).join(''):`<article class="studyCard"><div class="cardBody"><h3>${t('noStudiesH')}</h3><p>${t('noStudiesP')}</p></div></article>`;
